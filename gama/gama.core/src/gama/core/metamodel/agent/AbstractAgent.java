...
public abstract class AbstractAgent implements IAgent {
...

	{
		...
		return obj == null ? false : this.getUUID() == ((IAgent)obj).getUUID();
	}
	...
	public void setDirectVarValue(final IScope scope, final String s, final Object v) throws GamaRuntimeException {
		...
		//DEBUG.OUT("setDirectVarValue abstract agent " + s + " :: " + v);
	...
	public IPopulation<? extends IAgent> getPopulationFor(final ISpecies microSpecies) {
		...
		if (pop == null) {
			...
			// LG 30/07/2024
			// Comodeling fix, return the getExternMicroPopulationFor microSpecies properly when the previous getter failed
			if(pop == null)
			{	
				// MacroModel's context reaching for a species in the micro model
				var microPopulation = macroModelToMicroSpecies(microSpecies, micro);
	...
	private IPopulation<? extends IAgent> macroModelToMicroSpecies(final ISpecies microSpecies, final ModelDescription micro)
	{
		var microSetExpName = micro.getExperimentNames();
		for(var expName : microSetExpName)
		{
			var microExperimentPop = getHost().getExternMicroPopulationFor(micro.getMicroAlias() + "." + expName);
			if(microExperimentPop != null)
			{
				ExperimentPopulation microExpPopulation = (ExperimentPopulation) microExperimentPop.getPopulation(getScope());
				var microExperimentAgents = microExpPopulation.getAgents(getScope());
				ExperimentAgent microExperimentAgent0 = (ExperimentAgent)microExperimentAgents.firstValue(getScope());
				SimulationPopulation microSimulationPopulation0 = (SimulationPopulation) microExperimentAgent0.getMicroPopulations()[0];
				SimulationAgent microSimulationAgent0 = (SimulationAgent) microSimulationPopulation0.get(0);
