...
public record AgentReference(String[] species, Integer[] index, String cached_ref) implements IJsonable {
	...
	public IAgent getReferencedAgent(final IScope scope) {
		DEBUG.OUT("getReferencedAgent");
		if (scope == null) return null;
		IExperimentAgent sim = scope.getExperiment();
		if (sim == null) return null;
		IPopulation<? extends IAgent> pop = sim.getSimulationPopulation();
		IAgent referencedAgt = pop.getOrCreateAgent(scope, index[0], Collections.EMPTY_MAP);
		DEBUG.OUT("referencedAgt value is : " + referencedAgt);
		
		for (int i = 1; i < index.length; i++) {
			pop = referencedAgt.getPopulationFor(species[i]);
			if (pop == null) return null;
			referencedAgt = pop.getOrCreateAgent(scope, index[i], Collections.EMPTY_MAP);
			DEBUG.OUT("for in referencedAgt value is : " + referencedAgt);
			
		}
		DEBUG.OUT("returning referencedAgt " + referencedAgt);
		//DEBUG.OUT("returning referencedAgt attribute " + referencedAgt.getOrCreateAttributes());
